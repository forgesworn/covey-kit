...
- `src/hex.ts` — hex ⇄ bytes conversion (intentionally duplicated from roost-kit's own copy — kits stay independent)
...
## Release Notes
- Conventional commit prefixes matter: `fix:` for patch, `feat:` for minor. The package is pre-1.0 (currently `0.1.0`), so per SemVer a breaking change still bumps the minor (`0.x.0`) rather than major until the package reaches `1.0.0`.
